...
const TRACKING_JSON_PATH: &str = "./data/YOLOX_ncnn_palace/output.json";
...
impl Into<TrackingResultJson> for STrack {
    fn into(self) -> TrackingResultJson {
        TrackingResultJson {
            frame_id: (self.frame_id - 1).to_string(),
            track_id: self.track_id.to_string(),
            x: self.rect.x().to_string(),
            y: self.rect.y().to_string(),
            width: self.rect.width().to_string(),
            height: self.rect.height().to_string(),
        }
    }
}
...
fn export_byte_track_with_yolox() {
    ...
    let mut output_json = HashMap::new();
    ...
    for frame_id in 0..detection_results.len() {
        let objects = detection_results
            ...
            .collect();
        let outputs: Vec<TrackingResultJson> = byte_tracker
            .update(&objects)
            .iter()
            .map(|x| <STrack>::into(x.clone()))
            .collect();

        output_json.insert(frame_id, outputs);
    ...
    let output_json_path = "data/YOLOX_ncnn_palace/output.json";
    let output_json_file =
        std::fs::File::create(output_json_path).expect("Unable to create file");
    let output_vec = output_json
        .values()
        .flatten()
        .cloned()
        .collect::<Vec<TrackingResultJson>>();
    let traking_result = TrackingJson {
        name: "YOLOX_ncnn_palace".to_string(),
        fps: fps,
        track_buffer: track_buffer,
        results: output_vec,
    };
    serde_json::to_writer_pretty(output_json_file, &traking_result).unwrap();
}

fn main() {
    export_byte_track_with_yolox()
}
