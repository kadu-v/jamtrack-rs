...
class BoostTrack(object):
    def __init__(self, video_name: Optional[str] = None):
        ...
        if GeneralSettings['use_embedding']:
            if EmbeddingComputer is None:
                raise RuntimeError("Embedding is enabled but embedding dependencies are unavailable.")
...
class BoostTracker(object):
    def __init__(
        ...
        GeneralSettings.values["use_embedding"] = use_embedding
        GeneralSettings.values["use_ecc"] = use_ecc
        GeneralSettings.values["dataset"] = dataset
        GeneralSettings.values["test_dataset"] = test_dataset
        if det_thresh is not None:
            GeneralSettings.values["det_thresh"] = det_thresh
        if iou_threshold is not None:
            GeneralSettings.values["iou_threshold"] = iou_threshold
        if min_hits is not None:
            GeneralSettings.values["min_hits"] = min_hits
        if max_age is not None:
            GeneralSettings.values["max_age"] = max_age
        self.frame_rate = frame_rate
        self.tracker = BoostTrack(video_name=video_name)
        if max_age is not None:
            self.tracker.max_age = max_age

    def update(self, dets: np.ndarray, frame: np.ndarray, tag: str = "frame"):
        if frame is None:
            raise ValueError("BoostTracker.update requires a frame image.")
        ...
        img_tensor = np.zeros((1, 3, frame.shape[0], frame.shape[1]), dtype=np.float32)
        tracks = self.tracker.update(dets_in, img_tensor, frame, tag)
        if tracks.size == 0:
            return np.empty((0, 7), dtype=np.float32)

        outputs = []
        for x1, y1, x2, y2, track_id, conf in tracks.tolist():
            outputs.append([x1, y1, x2, y2, int(track_id), -1, conf])
